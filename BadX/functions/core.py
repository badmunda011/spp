...
class restrict:
    ...
    async def getRestrictedGroupList(self, message: Message):
        ...
        ReplyText = "**BadX - Restricted Group List**\n\n"
        Total: int = 0
...
class help_functions:
    ...
    async def send_alive(self, client: Client, message: Message):
        ...
        aliveText += f"➠ **BadX Version:** `{version['BadX']}`\n"
        ...
        if aliveMedia:
            ...
            try:
                ...
                await message.reply("**✅ BadX is alive** __(cannot send url or media here)__")
    ...
    async def is_restrictions(self, message: Message, user_id: int) -> bool:
        ...
        if message.from_user.id == OWNER_ID:
            if user_id in devs:
                await message.reply("__🫡 Sorry master but they're creator of BadX.__")
                return True
            else:
                return False
