...
@Client.on_message(
    filters.regex("🔸 Get All Sudos 🔸") & filters.private #& filters.user(TheBadX.sudo.sudoUsers)
)
async def get_all_sudos(_, message: Message):
    ...
    collection = []
    for sudo_user_id in TheBadX.sudo.sudoUsers:
        try:
            sudo_ = await TheBadX.BadX.get_users(int(sudo_user_id))
...
@Client.on_message(
    filters.regex("➕ Add Sudo") & filters.private #& filters.user(TheBadX.sudo.sudoUsers)
)
async def add_sudo_user(RiZoeL: Client, message: Message):
    ...
    if sudo_message.text.isnumeric() or sudo_message.text.startswith("@"):
        ...
        ask_rank: Message = await RiZoeL.ask(
            message.from_user.id,
            f"**❓ Please select sudo-rank for {sudo_user.mention}**\n\n **Sudo - Rank details**\n\n __1: Dev - Hold all rights. \n 2: Celestia - These users can access and verify data, as well as utilize BadX Spam Bots. \n 3: Apex - These users can only utilize BadX Spam Bots.__",
            filters=filters.text,
            timeout=120,
        ...
        if ask_rank.text in ["1: Dev", "2: Celestia", "3: Apex"]:
            ...
            await TheBadX.sudo.add(message, sudo_user.id, rank)
        else:
...
@Client.on_message(
    filters.regex("Remove Sudo ➖") & filters.private #& filters.user(TheBadX.sudo.sudoUsers)
)
async def remove_sudo_user(RiZoeL: Client, message: Message):
    if TheBadX.sudo.sudoFilter(message, 1):
        return
    sudo_message: Message = await RiZoeL.ask(
        message.from_user.id,
        "**❓ Please share user ID or username of sudo user!** __type /cancel to cancel the process!__",
    ...
    if sudo_message.text.isnumeric() or sudo_message.text.startswith("@"):
        ...
        await TheBadX.sudo.remove(message, sudo_user.id)
    else:
        await sudo_message.reply("**❕Please provide user ID or Username**")

@Client.on_message(
    filters.regex("Remove All ☑️") & filters.private #& filters.user(TheBadX.sudo.sudoUsers)
)
async def remove_all_sudo_user(RiZoeL: Client, message: Message):
    if message.from_user.id != TheBadX.owner_id:
    ...
    if confirm_message.text.lower() in ["yes", "no"]:
        if confirm_message.text.lower() == "yes":
            ...
            removed = 0
            for sudo_user_id in TheBadX.sudo.sudoUsers:
                await TheBadX.sudo.remove(confirm_message, int(sudo_user_id))
                removed += 1
            try:
...
async def sudoCallbacks(RiZoeL: Client, callback: CallbackQuery):
    ...
    if query[1] == "back":
        ...
        if int(query[2]) == TheBadX.owner_id:
        ...
        rank = TheBadX.sudo.sudos.get(int(query[2]))
        sudo_details += f" **- SudoRank: {rank} ({TheBadX.sudo.rankNames[rank]}) \n"

        promoted_by = TheBadX.database.getSudo(int(query[2]))
...
@Client.on_message(
    filters.regex("Active Tasks ℹ️") & filters.private #& filters.user(TheBadX.sudo.sudoUsers)
)
async def active_tasks(RiZoeL: Client, message: Message):
    if await TheBadX.sudo.sudoFilter(message, 2):
        return
    wait = await message.reply("▪ wait while get all active task.....")
    if len(TheBadX.activeTasks) == 0:
        active_message = None
    else:
        active_message = "🔹 All Active Task 🔹\n\n"
        active = 0
        for task_id in TheBadX.activeTasks:
            active_task = TheBadX.activeTasks.get(task_id)
            active += 1
    ...
    if len(TheBadX.functions.futures) == 0:
        future_message = None
    else:
        if active_message:
            active_message += "\n\n\n"
        ...
        future_active = 0
        for task_id in TheBadX.functions.futures:
            future_task = TheBadX.functions.futures.get(task_id)
            future_active += 1
            try:
                future_time = await TheBadX.functions.get_time(int(future_task['future']))
            except:
    ...
    await message.reply_document(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "📢 Updates", url=f"https://t.me/{TheBadX.updateChannel}"
                    ),
                    InlineKeyboardButton(
                        "Support 👥", url=f"https://t.me/{TheBadX.supportGroup}"
                    )
                ]
            ]
        )
...
@Client.on_message(
    filters.command("stop") & filters.private #& filters.user(TheBadX.sudo.sudoUsers)
)
async def stop_task(_, message: Message):
    ...
    if len(message.command) <= 1:
        ...
        if len(message.command) == 2:
            ...
            return
        if future:
            if active_id in TheBadX.functions.futures:
                TheBadX.functions.futures.pop(active_id)
            ...
            if active_id in TheBadX.activeTasks:
                if active_id in TheBadX.functions.unlimited:
                    TheBadX.functions.unlimited.remove(active_id)
                TheBadX.activeTasks.pop(active_id)
...
async def all_restricted_chat(_, message: Message):
    ...
    await TheBadX.restrict.getRestrictedGroupList(message)
...
async def add_chat(RiZoeL: Client, message: Message):
    ...
    if chat_details.text.startswith("-100") and chat_details.text.split("-")[1].isnumeric():
        await TheBadX.restrict.add(message, chat_details.text)
    else:
...
async def remove_chat(RiZoeL: Client, message: Message):
    ...
    if chat_details.text.startswith("-100") and chat_details.text.split("-")[1].isnumeric():
        if str(chat_details.text) == TheBadX.restrict.res:
            await chat_details.reply("__Sorry, You cannot remove that chat from Restricted List. It's support group of BadX__")
            return
        await TheBadX.restrict.remove(message, chat_details.text)
    else:
        await chat_details.reply("**❌ Invalid Chat ID!**")