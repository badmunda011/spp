...
from . import TheBadX
from BadX.functions.messages import start_message
from BadX.functions.keyboard import (
    start_keyboard,
    ...
    if len(message.command) > 1:
        ...
        if deep_cmd.lower() == "reboot":
            if await TheBadX.sudo.sudoFilter(message, 1):
                return
            await message.reply(
                "__Click below button to reboot the BadX!__",
                reply_markup=reboot_button,
            )
            return
        if deep_cmd.lower() == "help":
            await TheBadX.help(message.chat.id)
            return
    await message.reply(
...
@Client.on_message(
    filters.regex("🔹 Manage Clients 🔹") & filters.private #& filters.user(TheBadX.sudo.sudoUsers)
)
...
@Client.on_message(
    filters.regex("Other ↗️|🔙") & filters.private #& filters.user(TheBadX.sudo.sudoUsers)
)
...
@Client.on_message(
    filters.regex("👥 Sudo Users") & filters.private #& filters.user(TheBadX.sudo.sudoUsers)
)
async def sudo_users(_, message: Message):
    if await TheBadX.sudo.sudoFilter(message):
        return
    await message.reply(
        "**🔸 BadX - Sudo Panel (Please select)**",
...
async def restrictions(_, message: Message):
    if await TheBadX.sudo.sudoFilter(message):
        return
    await message.reply(
        "**🔸 BadX - Restriction Chat Panel (Please select)**",
...
@Client.on_message(
    filters.command("eval") #& filters.user(TheBadX.sudo.sudoUsers)
)
...
async def updateBadX(BadX: Client, message: Message):
    if await TheBadX.sudo.sudoFilter(message, 1):
        return
    try:
...
async def setvar(BadX: Client, message: Message):
    if await TheBadX.sudo.sudoFilter(message, 1):
        return
    ...
    if len(args) == 2:
        ...
        try:
            ...
            await message.reply_text(f"Error: {error} \n\n Report in @{TheBadX.supportGroup}")
    else:
        await message.reply_text(f"**Wrong Usage** \n Syntax: {TheBadX.handler}setvar (var name) (value)")